...
-module(antidote_crdt_ordered_map_rr).
...
-type state() :: gb_trees:tree(typedKey(), {NestedState::term()}).
-type op() ::
    {update, nested_op()}
  | {update, [nested_op()]}
  | {remove, typedKey()}
...
value(Map) ->
  [{{Key, Type}, Type:value(Value)} || {{Key, Type}, Value} <- gb_trees:to_list(Map)].
...
generate_downstream_update({{Key, Type}, Op}, Map) ->
  CurrentState =
    case gb_trees:is_defined({Key, Type}, Map) of
      true -> gb_trees:get({Key, Type}, Map);
      false -> Type:new()
    end,
...
generate_downstream_remove({Key, Type}, Map) ->
  CurrentState =
    case gb_trees:is_defined({Key, Type}, Map) of
      true -> gb_trees:get({Key, Type}, Map);
      false -> Type:new()
...
fold(_, Start, none) -> Start;
fold(F, Start, {K, V, I}) -> fold(F, F(K, V, Start), gb_trees:next(I)).
...
update({Updates, Removes}, State) ->
  ...
  Iter = gb_trees:iterator(State2),
  State3 = fold(fun(K, V, S) -> remove_obsolete(K, V, S)  end, new(), gb_trees:next(Iter)),
...
update_entry({{Key, Type}, {ok, Op}}, Map) ->
  case gb_trees:is_defined({Key, Type}, Map) of
    true ->
      State = gb_trees:get(Key, Map),
      {ok, UpdatedState} = Type:update(Op, State),
      gb_trees:update({Key, Type}, UpdatedState, Map);
    false ->
      NewValue = Type:new(),
      {ok, NewValueUpdated} = Type:update(Op, NewValue),
      gb_trees:insert({Key, Type}, NewValueUpdated, Map)
  end.

remove_entry({{Key, Type}, {ok, Op}}, Map) ->
  case gb_trees:is_defined({Key, Type}, Map) of
    true ->
      State = gb_trees:get({Key, Type}, Map),
      {ok, UpdatedState} = Type:update(Op, State),
      case is_bottom(Type, UpdatedState) of
        true ->
          gb_trees:delete({Key, Type}, Map);
        false ->
          gb_trees:insert({Key, Type}, UpdatedState, Map)
        end;
    false ->
        Map
  end;
...
remove_obsolete({Key, Type}, Val, Map) ->
  case is_bottom(Type, Val) of
    false ->
      gb_trees:insert({Key, Type}, Val, Map);
    true ->
      Map
  end.
